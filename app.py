...
# whisper-live sidecar (mic streaming for the SPA Live tab). Used by the
# /api/live/* routes that proxy to the streaming service.
...
# first; everything else falls through to the SPA catch-all mount. (The legacy
# Gradio UI was retired once the SPA reached parity — see git history.)
...
if not _HAS_SPA:
    log.warning(
        "SPA dist missing at %s — only the /api/* routes will be served. "
        "Run the ui build (see Dockerfile) to get the web UI.", UI_DIST,
    )
else:
    log.info("SPA dist found at %s — serving at /", UI_DIST)


# -- Launch --------------------------------------------------------------------
log.info("Launching whisper service on 0.0.0.0:7860...")
try:
    # Custom Starlette app: API routes + the SPA static mount at "/".
    import uvicorn
    ...
    # Mount the SPA LAST so it's the catch-all under "/" — /api/* is matched
    # first; everything else falls through to the static SPA (html=True serves
